...
export const getAnalyticsStats = async (
  monthYear?: string,
): Promise<{ data: AnalyticsStats | null; error: string | null }> => {
  try {
    ...
    const totalConsultations = filteredAppts.filter(
    ...
    const avgPatientsPerDoctor =
      doctorCount > 0 ? totalConsultations / doctorCount : 0;
    ...
    const returningPatients = Object.values(patientCounts).filter(
      (count) => count > 1,
    ).length;
    const patientReturnRate =
    ...
    const attendanceRate =
      totalRecords > 0 ? (attendancePointsTotal / totalRecords) * 100 : 0;

    // Build patient appointment list and returning patients list for completeness
    const patientAppointmentList = Object.entries(patientCounts).map(
      ([name, count]) => ({ name, count }),
    );
    const returningPatientsList = patientAppointmentList
...
export const getMonthlyPerformance = async (
  ...
  try {
    ...
    Object.keys(monthlyMetrics).forEach((month) => {
      ...
      appts.forEach(
        (
          appt: PatientRecord & { appointment_date: string; status: string },
        ) => {
          const fullName = getPatientFullName(appt);
          patientCounts[fullName] = (patientCounts[fullName] || 0) + 1;
        },
      ...
      monthlyMetrics[month].returnPatients = Object.values(
        patientCounts,
    ...
    const allConsultations = Object.values(monthlyMetrics).reduce(
      (sum, m) => sum + m.consultations,
      0,
    );
    const avgConsultations =
      allConsultations / Object.keys(monthlyMetrics).length || 1;

    let performanceData = Object.entries(monthlyMetrics)
      .map(([month, metrics]) => {
        // Normalize each metric to 0-100
        const consultationScore =
          (metrics.consultations / avgConsultations) * 100;
        const patientReturnScore =
          metrics.uniquePatients > 0
            ? (metrics.returnPatients / metrics.uniquePatients) * 100
            : 0;
        const attendanceScore =
          metrics.total > 0 ? (metrics.attended / metrics.total) * 100 : 0;

        // Combine scores (weight: consultations 40%, return rate 30%, attendance 30%)
        const combinedScore =
          consultationScore * 0.4 +
          patientReturnScore * 0.3 +
          attendanceScore * 0.3;
...
export const getWeeklyPerformance = async (
  ...
  try {
    ...
    const getWeekInfo = (
      dateStr: string,
    ): { weekNum: number; isInMonth: boolean } | null => {
      ...
      try {
        ...
        const weekNum = Math.ceil(
          (day + new Date(`${dateYear}-${dateMonth}-01`).getDay()) / 7,
        );
    ...
    Object.keys(weeklyMetrics).forEach((week) => {
      ...
      appts.forEach((appt: any) => {
        if (appt.patient_name) {
          patientCounts[appt.patient_name] =
            (patientCounts[appt.patient_name] || 0) + 1;
        }
      });

      weeklyMetrics[week].uniquePatients = Object.keys(patientCounts).length;
      weeklyMetrics[week].returnPatients = Object.values(patientCounts).filter(
        (count) => count > 1,
      ).length;
    });
    ...
    const allConsultations = Object.values(weeklyMetrics).reduce(
      (sum, m) => sum + m.consultations,
      0,
    );
    const avgConsultations =
      allConsultations / Object.keys(weeklyMetrics).length || 1;

    // Calculate average return rate and attendance rate for normalization
    const totalUniquePatients = Object.values(weeklyMetrics).reduce(
      (sum, m) => sum + m.uniquePatients,
      0,
    );
    const totalReturningPatients = Object.values(weeklyMetrics).reduce(
      (sum, m) => sum + m.returnPatients,
      0,
    );
    const avgReturnRate =
      totalUniquePatients > 0
        ? (totalReturningPatients / totalUniquePatients) * 100
        : 0;

    const totalAttendance = Object.values(weeklyMetrics).reduce(
      (sum, m) => sum + m.total,
      0,
    );
    const totalPresent = Object.values(weeklyMetrics).reduce(
      (sum, m) => sum + m.attended,
      0,
    );
    ...
    const performanceData = Object.entries(weeklyMetrics)
      .map(([week, metrics]) => {
        ...
        const weekReturnRate =
          metrics.uniquePatients > 0
            ? (metrics.returnPatients / metrics.uniquePatients) * 100
            : 0;
        const patientReturnScore =
          avgReturnRate > 0 ? (weekReturnRate / avgReturnRate) * 100 : 0;
        ...
        const attendanceScore =
          avgAttendanceRate > 0
            ? (weekAttendanceRate / avgAttendanceRate) * 100
            : 0;
