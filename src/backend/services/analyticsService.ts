...
export const getAnalyticsStats = async (): Promise<{ data: AnalyticsStats | null; error: string | null }> => {
  try {
    ...
    const totalConsultations = allAppts?.length ?? 0;
    ...
    allAppts?.forEach((appt: { patient_name: string }) => {
    ...
    // If you want the actual list of returning patients:
    // const returningPatientsList = Object.entries(patientCounts).filter(([_, count]) => count > 1).map(([name]) => name);
 ...
 */
export const getMonthlyPerformance = async (): Promise<{ data: { month: string, value: number }[] | null; error: string | null }> => {
  try {
    ...
    Object.keys(monthlyMetrics).forEach(month => {
      const appts = allAppts?.filter(a => {
        ...
        const aMonth = date.toLocaleString('default', { month: 'short', year: 'numeric' });
        return aMonth === month;
      }) ?? [];
    ...
    const performanceData = Object.entries(monthlyMetrics)
      .map(([month, metrics]) => {
